...
export function setSessionData(key, value) {
  ...
  } else if (!isString(data) && data.toString) {
    data = data.toString();
  }
  sessionStorage.setItem(key, data);
}

export function getSessionData(key, callback) {
...
export function parseUploadFileUrl(path) {
  if (path) {
    return `http://static.pinwheelmedical.com/${[path]}`;
  } else {
    return '';
  }
}
...
// 补位
export function padLeft(total, pad) {
  return (Array(pad).join(0) + total).slice(-pad);
}
...
export class MathTool {
  static sum(a, b) {
    ...
    const r1 = a % 1 === 0 ? 0 : a.toString().split('.')[1].length;
    const r2 = b % 1 === 0 ? 0 : b.toString().split('.')[1].length;
