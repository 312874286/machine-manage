    ...
    const columns = [
      ...
      },
      {
        ...
        render: (text, item) => (
          <Fragment>
            <a
              onClick={() => this.props.history.push({ pathname: '/check/fault', query: { flag: 'openFault', machine: `${item.local}—${item.machineCode}`, machineId: `${item.id}、${item.machineCode}`, type: 3,}})}
