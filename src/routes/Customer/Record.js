...
import RecordTable from '../../components/Customer/recordTable';
...
const CollectionCreateForm = Form.create()(
  (props) => {
    ...
    // const { name, userName, sex, birthday } = modalData;
...
@connect(({ record, loading, log }) => ({
  record,
  log,
  loading: loading.models.record,
}))
@Form.create()
export default class CustomerRecord extends PureComponent {
  state = {
    pageNo: 1,
    keyword: '',
  ...
  render() {
    ...
    const { record: { list, page }, log: { logList, logPage }, loading } = this.props;
                  ...
                  <Col md={8} sm={12}>
                    <FormItem label="关键字">
                      {getFieldDecorator('keyword', {
                        ...
                        <Input placeholder="请输入档案ID、儿童姓名" />
                      )}
                    </FormItem>
                  </Col>
                  <Col md={8} sm={12}>
